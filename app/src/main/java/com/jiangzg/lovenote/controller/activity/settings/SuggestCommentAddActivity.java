...
public class SuggestCommentAddActivity extends BaseActivity<SuggestCommentAddActivity> {
    ...
    public static void goActivity(Activity from, long sid) {
        if (sid == 0) return;
        Intent intent = new Intent(from, SuggestCommentAddActivity.class);
    ...
    protected int getView(Intent intent) {
        return R.layout.activity_suggest_comment_add;
    }
    ...
    private void onContentInput(String input) {
        if (limitContentLength <= 0) {
            limitContentLength = SPHelper.getLimit().getSuggestCommentContentLength();
        }
